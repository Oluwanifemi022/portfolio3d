...
from utils.storage import save_images, save_models, save_image, delete_file, get_display_url
...
st.header("Gallery")
st.caption("Standalone images with no 3D file attached — just a look, like a photo feed.")

gallery_files = st.file_uploader(
    "Add images to the gallery (you can select several)",
    type=["png", "jpg", "jpeg", "webp"],
    accept_multiple_files=True,
    ...
    if not gallery_files:
        ...
        try:
            for f in gallery_files:
                path = save_image(f)
                add_gallery_image(path)
            st.success(f"Added {len(gallery_files)} image(s) to the gallery.")
        except Exception as e:
            st.error("Uploading to the gallery failed. Full error below:")
            st.exception(e)

gallery_images = get_gallery_images()
if gallery_images:
    st.caption(f"{len(gallery_images)} image(s) in the gallery:")
    gcols = st.columns(6)
    for i, img in enumerate(gallery_images):
        with gcols[i % 6]:
            ...
            if st.button("Delete", key=f"del_gallery_{img['id']}"):
                deleted = delete_gallery_image(img["id"])
                if deleted:
...
with st.expander("Debug info"):
    if DB_PATH:
        ...
        st.write("Using Supabase - no local database file.")
    st.write("Item count reported by get_all_items():", len(items))
    st.write("Gallery image count:", len(gallery_images))
