...
# System temp dir rather than a path inside the app's own folder - on
# Streamlit Community Cloud the cloned repo folder can be read-only,
# so writing the SQLite file there can fail. The OS temp dir is
# always writable, both on Streamlit Cloud and locally. Still
# EPHEMERAL either way - see README for moving to hosted Postgres.
DB_PATH = os.path.join(tempfile.gettempdir(), "portfolio3d_uploads", "portfolio.db")
...
def add_gallery_image(path: str, caption: str = None) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO gallery_images (path, caption, created_at) VALUES (?, ?, ?)",
            (path, caption, datetime.now(timezone.utc).isoformat()),
        )
...
def delete_gallery_image(image_id: int):
    with get_conn() as conn:
        row = conn.execute(
            "SELECT path FROM gallery_images WHERE id = ?", (image_id,)
        ).fetchone()
